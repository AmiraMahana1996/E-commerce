...
export default function Explaination() {
  ...
  const handleSubmit = async (event) => {
    ...
    await axios.put(
      `http://localhost:5000/api/learning/addSubtopicexplanation/${path[4]}/${path[5]}/${path[6]}`,
      formData
    );
  ...
  return (
    <>
      ...
      <br />
      <br />
      {data?.explaination?.map((explaination) => (
        <Card variant="soft">
          <CardContent>
            ...
            <Typography color="sucsess"> {explaination.example}</Typography>
          </CardContent>
        </Card>
      ))}
